...
export class FussballComponent implements OnInit {
  ...
  loadReservations(): void {
    ...
    this.http.get<any[]>(backend_url_base + 'reservations?type=fussball').subscribe(data => {
    this.blocks = data
  ...
  inviteGuest(): void {
    ...
    // 💥 NU permite trimiterea către un user deja invitat
    const alreadyInvited = this.selectedReservation.guests?.some(
      g => this.users.find(u => u.id === this.invitedUserId)?.email === g.email
    );

    if (alreadyInvited) {
      alert('Acest utilizator a fost deja invitat.');
      return;
    }

    // ✅ Trimite invitația doar dacă NU e deja invitat
    this.http.post(`${backend_url_base}reservations/${this.selectedReservation.id}/invite`, {
      ...
      error: () => alert('Eroare la trimiterea invitației.')
    });
  }
